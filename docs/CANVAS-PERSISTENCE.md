...
本文描述 Task-Centric Canvas V2 已实现的事实源、command 保存协议、冲突分支、运行记录与 Canvas Git 边界。V1 的整文档 `PUT /canvas`、node-owned session 和按路径读取 artifact 不属于 V2 数据流。

## 事实源与派生状态

| 层 | 路径或存储 | 职责 | 是否事实源 |
| --- | --- | --- | --- |
| 浏览器视图 | IndexedDB `ggai-canvas-v2` / `branch-view-state` | camera、selection、Task/Collection 折叠、composer draft | 否，仅 branch-local 视图 |
...
`CanvasDocumentV2` 只保存 Task、Node、Collection、typed Edge、materialization/proposal receipt 和 `everCreated`。active Run、SSE cursor、日志缓存、视图状态、outbox、mutation ledger、artifact bytes 与原始 JSONL 不进入 Canvas Git。

## Command 保存与 exactly-once

V2 不允许浏览器上传整份 Canvas snapshot。所有持久语义变更走：
...
daemon 在 runtime snapshot 内保存 `mutationId → commandDigest + committedRevision` 的 exactly-once ledger。它不会返回给浏览器，也不会进入 `CanvasDocumentV2` 或 Canvas Git。相同 `mutationId` 与相同 command 即使在后续 revision、daemon 重启或 HTTP 成功响应丢失后重试，也返回当前规范 envelope，不再执行副作用；同一 ID 携带不同 command 会被拒绝。
...
刷新时先读取 `GET /canvas/v2`，再加载 branch-local 视图和 outbox，并对 outbox 重放乐观 reducer。Task runtime 不从 Canvas 文档伪造，而是通过 Task Run 历史、永久日志和 SSE 恢复：
...
编码和解码都运行完整 `CanvasDocumentV2` 语义校验。新建分支前 checkpoint 来源分支；历史恢复总是创建新分支；merge preview 固定 source/target commit 与目标 runtime revision，确认后任一值变化都会要求重新预览。合并结果在写入 runtime 前再次解码和校验，拒绝 dangling ref、非法嵌套、重复 receipt 或不一致的 Agent origin。

`main` 受保护，分支操作由 daemon 管理的 UUID worktree 执行，不切换用户源码 checkout。V2 Canvas Git 与用户源码 Git 物理隔离；旧 source-binding HTTP 路径不属于 V2 协议。

## HTTP 接口速查

| 接口 | V2 用途 |
| --- | --- |
| `GET /health` | 确认 `canvasModelV2`、schema 2 与 reset 状态 |
| `GET /canvas/v2` | 加载指定分支 envelope |
...
| `PUT /plugin-capabilities/v2` | 注册 data-only artifact claim 快照并取得 digest |
| `POST /runs` | 使用 `RunIntentV2` 启动 Task-owned Run |
...
- V2 不读取、不迁移 V1 Canvas，也没有旧画布查看器。按 [`CANVAS-V2-RESET.md`](./CANVAS-V2-RESET.md) 先预览，再显式运行 `npm run canvas:v2:reset -- --apply`；脚本把旧状态移动到 `.gg/legacy-v1/<timestamp>/`，不物理删除。
- `.gg/` 和 `artifacts/` 必须保持源码 Git ignored；reset 在目标中发现 tracked file 会拒绝执行。
- 正常 `Ctrl+C` 会释放项目 lease。遇到 stale lease 不要直接删除锁；停止并确认旧 daemon 已退出后，使用显式 V2 reset 归档旧 runtime，或按错误提示进行人工核验。
- 当前 UI 与 daemon 均为 V2-only。V1 状态只存在于 reset 归档中，应用不会读取、查看、迁移或写回它。
