...
- 推荐基线：Node.js 22.22.3、npm 10.9.8（见 `.nvmrc` 与 `packageManager`）。
- 支持范围：Node.js 22.22+ 或 Node.js 24 LTS；npm 10.9+ 或 npm 11。
- Node 22.22 是 React Router 8 的最低要求，也覆盖 acpx 的 Node 22.13 最低要求。
...
- `dependencies`：浏览器生产代码和 daemon 运行时直接加载的包。`chokidar` 属于 daemon 运行时依赖。
...
`npm run daemon` 是开发入口，会先编译 TypeScript；`npm run daemon:start` 只启动已经生成的 `dist-daemon`。生产交付使用 `npm run package:daemon`：它排除测试与 source map，并从根 lockfile 确定性裁剪出独立的 runtime manifest/lockfile，不访问 registry 重新解析版本。当前闭包只有 `chokidar` 与 `readdirp`；runtime 允许正常执行依赖脚本，不会携带 React、Vite 或 TypeScript。
