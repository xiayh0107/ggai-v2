...
- `dependencies`：浏览器生产代码和 daemon 运行时直接加载的包。`chokidar` 与 Ajv 属于 daemon 运行时依赖；`node:sqlite` 是 Node 24 内置模块，不进入 npm manifest。
...
`npm run daemon` 是开发入口，会先编译 TypeScript；`npm run daemon:start` 只启动已经生成的 `dist-daemon`。生产交付使用 `npm run package:daemon`：它排除测试与 source map，并从根 lockfile 确定性裁剪出独立的 runtime manifest/lockfile，不访问 registry 重新解析版本。npm 闭包包含 Chokidar、Ajv 及其直接运行依赖，SQLite 由 Node 24 提供；runtime 允许正常执行依赖脚本，不会携带 React、Vite 或 TypeScript。
