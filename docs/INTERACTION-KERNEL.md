...
本文定义 Task-Centric Canvas 与外部 Agent 之间最小、可信的语义边界。完整实体和 command 规范见 [`CANVAS.md`](./CANVAS.md)，节点插件与上下文映射见 [`PLUGIN-CONTEXT-CONTRACT.md`](./PLUGIN-CONTEXT-CONTRACT.md)。内核不实现 Agent 调度器、Agent 间消息总线，也不允许 Agent 直接修改画布。
...
用户 command → 持久 Task + Canvas revision
                         │
                         ▼
                RunIntent(taskId, revision)
                         │
                         ▼
            events / file-write ghost progress
                         │
                         ▼ durable close
       ArtifactManifest + validated RunOutcome
                         │
                         ▼
...
- Agent 可以写 deliverable 和受限 `RunOutcome` sidecar，但不能声明 runId、Task/Node ID、payload、自由 edge、坐标或后续自动执行。
...
- run log 保存终态 close 与恢复证据，不能独立删除。
...
artifact claim 与 Node UI template 都是 JSON 可序列化数据；插件没有 React renderer 或投影函数。浏览器把启用的 community claims 注册给 daemon，daemon 合并不可覆盖的内置 registry、规范化并返回 content digest。Run 接受时把完整能力快照固定下来，并写入上下文包。

Agent outcome 的 `pluginId` 必须属于该固定快照，path/MIME 也必须满足对应 claim。daemon 将 outcome、manifest 与 claims 求交集；未知文件只由 daemon-owned `file` fallback 接收。浏览器拿到 manifest-backed metadata 后，才由平台按相同 plugin 的有限模板渲染。
...
- 新 Node 类型通过 data-only artifact claim 与 UI template 扩展，不在核心按 plugin ID 分支。
- 新 Edge relation 不得隐式改变上下文；每条命令必须继续显式携带
  ...
  的普通连接为 `references/none`。从来源端口直接创建空 Node 是派生创建快捷方式，保存
  `source/full`；该空 Node 升级为 Task 输出槽时，共享 reducer 必须把有效入边原子提升为
  直接 Task 输入。`produced/derived` 只来自可信物化路径。
