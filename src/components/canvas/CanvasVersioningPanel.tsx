...
import type { CanvasSaveConflictBranchResult } from '@/canvas/daemonClient'
import type {
  CanvasHistoryEntry,
  CanvasVersionBranch,
  CanvasVersioningClient,
  CanvasVersioningStatus,
  CanvasWorkspaceMergePreview,
  CanvasWorkspaceOperationResult,
} from '@/canvas/versioningClient'

export interface CanvasVersioningFlushStore {
  flushCommands(): Promise<void>
  saveConflictAsBranch(newBranch: string): Promise<CanvasSaveConflictBranchResult>
  getSnapshot(): Pick<CanvasStoreState, 'commandSync'>
}

export interface CanvasVersioningPanelProps {
  client: Pick<
    CanvasVersioningClient,
    | 'status'
    | 'listBranches'
  ...
  projectDir: string
  branch: string
  store: CanvasVersioningFlushStore
...
export default function CanvasVersioningPanel({
  client,
  projectDir,
  branch,
  store,
  onNavigateBranch,
  onClose,
}: CanvasVersioningPanelProps) {
  const headingId = useId()
  const [status, setStatus] = useState<CanvasVersioningStatus | null>(null)
  const [branches, setBranches] = useState<CanvasVersionBranch[]>([])
  ...
  const [mergePreview, setMergePreview] = useState<CanvasWorkspaceMergePreview | null>(null)
    ...
    <>
      <Dialog open onOpenChange={(open) => {
        ...
        <DialogContent
          ...
          data-testid="canvas-versioning-panel"
        >
          <DialogHeader className="border-b border-gg-line px-6 py-5 text-left">
            ...
            <DialogDescription className="text-[12px] leading-5 text-gg-muted">
              仅记录 Canvas 文档。任务运行、日志、源码与本地视图状态不在此历史中。
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={mergePreview !== null} onOpenChange={(open) => {
        ...
        <AlertDialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto border-gg-line bg-gg-node text-gg-ink">
          <AlertDialogHeader>
            <AlertDialogTitle>确认合并 Canvas 分支</AlertDialogTitle>
            <AlertDialogDescription className="text-gg-muted">
...
function StatusBadge({ status }: { status: CanvasVersioningStatus | null }) {
...
function operationFailure<Value>(
  label: string,
  result: Extract<CanvasWorkspaceOperationResult<Value>, { ok: false }>,
): string {
...
function deduplicateHistory(entries: CanvasHistoryEntry[]): CanvasHistoryEntry[] {
