...
import CanvasVersioningPanel, {
  type CanvasVersioningFlushStore,
} from './CanvasVersioningPanel'
...
function syncedStore(overrides: Partial<CanvasStoreState['commandSync']> = {}) {
  const commandSync: CanvasStoreState['commandSync'] = {
    status: 'saved',
    pendingCount: 0,
    error: null,
    conflict: null,
  ...
  } satisfies CanvasVersioningFlushStore
}

function mergePreview(): CanvasWorkspaceMergePreview {
  return {
    state: 'ready',
    canvas: {
...
describe('CanvasVersioningPanel', () => {
  it('is an accessible current panel with branches, checkpoints, and no source/delete controls', async () => {
    await renderSubject()

    const dialog = document.querySelector<HTMLElement>('[role="dialog"]')
    expect(dialog).not.toBeNull()
    expect(dialog?.getAttribute('aria-labelledby')).toBeTruthy()
    expect(dialog?.textContent).toContain('仅记录 Canvas 文档')
