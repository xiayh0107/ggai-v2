...
import {
  ...
  type TaskProposalEditsV2,
  type TrustedTaskProposalV2,
} from '@/canvas-v2/commands'

interface ProposalDraftV2 {
  title: string
  prompt: string
  dependsOn: string[]
}

export interface CanvasV2ProposalReviewAcceptance {
  proposalKeys: string[]
  edits: TaskProposalEditsV2
}

export interface CanvasV2ProposalReviewPanelProps {
  proposals: readonly TrustedTaskProposalV2[]
  settledProposalKeys?: readonly string[]
  onAccept: (acceptance: CanvasV2ProposalReviewAcceptance) => void
  onReject: (proposalKeys: string[]) => void
}

interface ProposalReviewEditorProps extends CanvasV2ProposalReviewPanelProps {
  unsettledProposals: readonly TrustedTaskProposalV2[]
}
 ...
 * caller.
 */
export default function CanvasV2ProposalReviewPanel(
  props: CanvasV2ProposalReviewPanelProps,
) {
...
function ProposalReviewEditor({
  ...
  const [drafts, setDrafts] = useState<Record<string, ProposalDraftV2>>(() =>
  ...
  return (
    <section
      data-testid="canvas-v2-proposal-review-panel"
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <input
                          ...
                          value={draft.title}
                          disabled={!selected}
                          maxLength={MAX_TASK_PROPOSAL_EDIT_TITLE_LENGTH_V2}
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          disabled={!selected}
                          rows={3}
                          maxLength={MAX_TASK_PROPOSAL_EDIT_PROMPT_LENGTH_V2}
...
function validateDrafts(
  ...
  drafts: Readonly<Record<string, ProposalDraftV2>>,
  proposalsByKey: ReadonlyMap<string, TrustedTaskProposalV2>,
): string[] {
  ...
  for (const proposalKey of order) {
    ...
    if (draft.dependsOn.length > MAX_TASK_PROPOSAL_EDIT_DEPENDENCIES_V2) {
      issues.push(`${proposal.title}：前置任务不能超过 ${MAX_TASK_PROPOSAL_EDIT_DEPENDENCIES_V2} 个。`)
    }
...
function buildEdits(
  ...
  const edits: TaskProposalEditsV2 = {}
  for (const proposalKey of proposalKeys) {
    const proposal = proposalsByKey.get(proposalKey)
    const draft = drafts[proposalKey]
    if (!proposal || !draft) continue
    const edit: TaskProposalEditV2 = {}
