...
import type { CanvasV2RunIntent } from '@/canvas-v2/runController'
import {
  DaemonClient,
  DaemonClientError,
  DaemonProtocolError,
...
function intent(overrides: Partial<CanvasV2RunIntent> = {}): CanvasV2RunIntent {
  return {
    schemaVersion: 2,
    runId: 'run-v2',
    taskId: 'task-v2',
...
describe('DaemonTaskRunClientV2', () => {
  ...
  ])('rejects a %s taskId in a V2 run summary', async (_label, taskId) => {
  ...
  it('reconciles network uncertainty only by GET runId and never sends cancellation on 404', async () => {
    ...
    try {
      ...
      expect(createError).toBeInstanceOf(DaemonClientError)
  ...
  it('bounds a pending V2 create and leaves timeout reconciliation to the controller', async () => {
    vi.useFakeTimers()
    try {
