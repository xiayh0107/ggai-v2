...
export const RUN_OUTCOME_V2_SCHEMA_VERSION = 2
export const MAX_RUN_OUTPUT_HINTS_V2 = 32
export const MAX_RUN_TASK_PROPOSALS_V2 = 12
export const MAX_RUN_OUTPUT_KEY_LENGTH_V2 = 80
export const MAX_RUN_OUTPUT_PATH_LENGTH_V2 = 4_096
export const MAX_RUN_OUTPUT_PLUGIN_ID_LENGTH_V2 = 160
export const MAX_RUN_OUTPUT_TITLE_LENGTH_V2 = 240
export const MAX_RUN_OUTPUT_PARENTS_V2 = 8
export const MAX_RUN_TASK_PROPOSAL_DEPENDENCIES_V2 = 8
export const MAX_RUN_TASK_PROPOSAL_INPUTS_V2 = 32
export const MAX_RUN_TASK_PROPOSAL_TITLE_LENGTH_V2 = 240
export const MAX_RUN_TASK_PROPOSAL_PROMPT_LENGTH_V2 = 10_000

export type RunOutputRoleV2 = 'primary' | 'supporting' | 'auxiliary'
 ...
 */
export interface RunOutputHintV2 {
  key: string
 ...
 */
export interface RunTaskProposalV2 {
  key: string
  title: string
  prompt: string
...
export interface RunOutcomeV2 {
  ...
  outputs: RunOutputHintV2[]
  taskProposals: RunTaskProposalV2[]
}

export type RunOutcomeV2Inspection =
  | { status: 'valid'; outcome: RunOutcomeV2 }
  | { status: 'unsupported' }
  | { status: 'invalid'; reason: string }

/** Browser-safe POSIX path relative to a run's `files/` directory. */
export function isSafeRunRelativeArtifactPathV2(value: unknown): value is string {
  if (
    typeof value !== 'string'
    || value.length === 0
    || value.length > MAX_RUN_OUTPUT_PATH_LENGTH_V2
    || value.startsWith('/')
    || value.endsWith('/')
    || value.includes('\\')
    || value.includes('\0')
...
export function inspectRunOutcomeV2(value: unknown): RunOutcomeV2Inspection {
  ...
  if (value.schemaVersion !== RUN_OUTCOME_V2_SCHEMA_VERSION) return { status: 'unsupported' }
  ...
  if (!Array.isArray(value.outputs) || value.outputs.length > MAX_RUN_OUTPUT_HINTS_V2) {
    return invalid('outputs exceeds the supported bound')
  }
  if (!Array.isArray(value.taskProposals)
    || value.taskProposals.length > MAX_RUN_TASK_PROPOSALS_V2) {
    return invalid('taskProposals exceeds the supported bound')
  }

  const outputs: RunOutputHintV2[] = []
  ...
  const taskProposals: RunTaskProposalV2[] = []
  ...
  return {
    status: 'valid',
    outcome: {
      schemaVersion: RUN_OUTCOME_V2_SCHEMA_VERSION,
      suggestedActions,
      outputs,
      taskProposals,
    },
...
function inspectOutputHint(
  value: unknown,
  index: number,
): { status: 'valid'; output: RunOutputHintV2 } | { status: 'invalid'; reason: string } {
  ...
  if (!isStableKey(value.key, MAX_RUN_OUTPUT_KEY_LENGTH_V2)) {
    return invalid(`${label}.key is invalid`)
  }
  if (!isSafeRunRelativeArtifactPathV2(value.path)) return invalid(`${label}.path is unsafe`)
  ...
  if (value.title !== undefined && !isDisplayString(value.title, MAX_RUN_OUTPUT_TITLE_LENGTH_V2)) {
    return invalid(`${label}.title is invalid`)
  }
  if (value.derivedFrom !== undefined && (
    !Array.isArray(value.derivedFrom)
    || value.derivedFrom.length > MAX_RUN_OUTPUT_PARENTS_V2
    || !value.derivedFrom.every((key) => isStableKey(key, MAX_RUN_OUTPUT_KEY_LENGTH_V2))
...
function inspectTaskProposal(
  value: unknown,
  index: number,
): { status: 'valid'; proposal: RunTaskProposalV2 } | { status: 'invalid'; reason: string } {
  ...
  if (!isDisplayString(value.title, MAX_RUN_TASK_PROPOSAL_TITLE_LENGTH_V2)) {
    return invalid(`${label}.title is invalid`)
  }
  if (!isDisplayString(value.prompt, MAX_RUN_TASK_PROPOSAL_PROMPT_LENGTH_V2)) {
    return invalid(`${label}.prompt is invalid`)
  }
  if (!Array.isArray(value.inputOutputKeys)
    || value.inputOutputKeys.length > MAX_RUN_TASK_PROPOSAL_INPUTS_V2
    || !value.inputOutputKeys.every((key) => isStableKey(key, MAX_RUN_OUTPUT_KEY_LENGTH_V2))
  ...
  if (value.dependsOn !== undefined && (
    ...
    || value.dependsOn.length > MAX_RUN_TASK_PROPOSAL_DEPENDENCIES_V2
    || !value.dependsOn.every((key) => isStableKey(key, MAX_RUN_OUTPUT_KEY_LENGTH_V2))
...
function isPluginId(value: unknown): value is string {
  return typeof value === 'string'
    && value.length > 0
    && value.length <= MAX_RUN_OUTPUT_PLUGIN_ID_LENGTH_V2
    && /^@?[A-Za-z0-9][A-Za-z0-9._:@/-]*$/u.test(value)
    && !value.includes('..')
    && !value.includes('//')
}

function isOutputRole(value: unknown): value is RunOutputRoleV2 {
