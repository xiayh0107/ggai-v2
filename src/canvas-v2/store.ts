...
import {
  ...
  type CanvasV2SelectionTarget,
  type CanvasV2ViewState,
} from './persistence'
import {
  selectTaskViewV2,
  type CanvasGhostOutputV2,
  type CanvasProposalReviewV2,
  type CanvasTaskRuntimeV2,
  type CanvasTaskViewV2,
} from './selectors'

export type CanvasV2HydrationStatus = 'idle' | 'loading' | 'ready' | 'error'
export type CanvasV2CommandSyncStatus =
  | 'idle'
  | 'pending'
  | 'saving'
  | 'saved'
  | 'error'
  | 'conflict'
export type CanvasV2ViewSyncStatus = 'idle' | 'pending' | 'saving' | 'saved' | 'error'

export interface CanvasV2StoreConflict {
...
export interface CanvasV2StoreState {
  scope: CanvasV2CanvasScope
  hydration: {
    status: CanvasV2HydrationStatus
    error: string | null
  }
  commandSync: {
    status: CanvasV2CommandSyncStatus
    pendingCount: number
    error: string | null
    conflict: CanvasV2StoreConflict | null
  }
  viewSync: {
    status: CanvasV2ViewSyncStatus
    error: string | null
  }
...
export interface CanvasV2StoreClient {
  getCanvas(scope: CanvasV2CanvasScope): Promise<CanvasV2Envelope>
  flushOutbox(
    scope: CanvasV2CanvasScope,
...
export interface CanvasV2StoreOptions {
  ...
  scope: CanvasV2CanvasScope
  client: CanvasV2StoreClient | CanvasV2DaemonClient
  persistence?: CanvasV2Persistence<CanvasCommandV2>
  mutationId?: () => string
  viewWriteDelayMs?: number
}

export type CanvasV2OutboxReplayResult =
  | { status: 'applied'; document: CanvasDocumentV2 }
  | {
      status: 'conflict'
      document: CanvasDocumentV2
      mutationId: string
...
export function defaultCanvasV2ViewState(): CanvasV2ViewState {
  return {
...
export function replayCanvasV2Outbox(
  document: CanvasDocumentV2,
  entries: readonly CanvasV2OutboxEntry<CanvasCommandV2>[],
): CanvasV2OutboxReplayResult {
  let next = document
  for (const entry of entries) {
    try {
      next = applyCanvasCommandV2(next, entry.command)
    } catch (error) {
      return {
        status: 'conflict',
        document: next,
...
export class CanvasV2Store {
  readonly #client: CanvasV2StoreClient
  readonly #persistence: CanvasV2Persistence<CanvasCommandV2>
  readonly #outbox: CanvasV2CommandOutbox
  readonly #persistenceScope: CanvasV2PersistenceScope
  ...
  constructor(options: CanvasV2StoreOptions) {
    ...
    if (!options.scope.projectDir || !options.scope.branch) {
      throw new TypeError('Canvas V2 scope requires projectDir and branch')
    }
    this.#client = options.client
    this.#persistence = options.persistence ?? new CanvasV2Persistence<CanvasCommandV2>()
    this.#outbox = {
  ...
  readonly getSnapshot = (): CanvasV2StoreState => this.#state
  ...
  dispatchCommand(command: CanvasCommandV2): Promise<{ mutationId: string }> {
    ...
    const operation = this.#enqueueTail.then(async () => {
      ...
      applyCanvasCommandV2(this.#state.document, command)
      ...
      await this.#withOutboxLock(async () => {
        ...
        const nextDocument = applyCanvasCommandV2(this.#state.document, command)
  ...
  setCamera(camera: CanvasV2ViewState['camera']): void {
    if (![camera.x, camera.y, camera.zoom].every(Number.isFinite) || camera.zoom <= 0) {
      throw new TypeError('Canvas V2 camera is invalid')
    }
  ...
  setComposerDraft(entityKey: string, draft: string): void {
    if (!entityKey || entityKey.length > 512 || draft.length > 250_000) {
      throw new TypeError('Canvas V2 composer draft is invalid')
    }
    this.#updateView((view) => ({
      ...view,
  ...
  setTaskRuntime(runtime: CanvasTaskRuntimeV2): void {
    ...
    if (new Set(keys).size !== keys.length) {
      throw new TypeError('Canvas V2 ghost keys must be unique per task')
    }
    this.#setState((state) => ({
      ...state,
      runtimeByTaskId: {
  ...
  selectTaskView(
    ...
    return selectTaskViewV2(this.#state.document, taskId, {
  ...
  async #load(preserveLiveView: boolean): Promise<void> {
    ...
    try {
      ...
      const replay = replayCanvasV2Outbox(envelope.document, entries)
  ...
  async #flushCommandsOnce(): Promise<void> {
    ...
    try {
      ...
      if (!envelope) throw new Error('Canvas V2 flush did not return an envelope')
      await this.#withOutboxLock(async () => {
        this.#acknowledgedRevision = envelope.revision
        const pending = await this.#persistence.list(this.#persistenceScope)
        const replay = replayCanvasV2Outbox(envelope.document, pending)
        if (replay.status === 'conflict') {
          this.#setState((state) => ({
            ...state,
            envelope,
  ...
  #assertCanDispatch(): void {
    if (this.#state.hydration.status !== 'ready') {
      throw new Error('Canvas V2 is not ready')
    }
    if (this.#state.commandSync.status === 'conflict') {
      throw new Error('Canvas V2 has an unresolved command conflict')
    }
    if (this.#state.commandSync.status === 'error') {
      throw new Error('Canvas V2 command outbox must be retried first')
    }
  }

  #updateView(update: (view: CanvasV2ViewState) => CanvasV2ViewState): void {
  ...
  #setState(update: (state: CanvasV2StoreState) => CanvasV2StoreState): void {
