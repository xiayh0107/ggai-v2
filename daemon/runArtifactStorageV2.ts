...
import {
  ...
  buildArtifactManifestV1,
  inspectArtifactManifestV1,
  isArtifactControlRelativePathV1,
  isTemporaryArtifactRelativePathV1,
  MAX_ARTIFACT_MANIFEST_ENTRIES_V1,
  normalizeArtifactRelativePathV1,
  type ArtifactManifestEntryV1,
  type ArtifactManifestFileCandidateV1,
...
export const RUN_ARTIFACT_MANIFEST_RELATIVE_PATH_V2 = '.ggai/artifact-manifest.v1.json'
export const MAX_RUN_ARTIFACT_DIRECTORIES_V2 = 2_000
export const MAX_RUN_ARTIFACT_DEPTH_V2 = 64
export const MAX_RUN_ARTIFACT_MANIFEST_BYTES_V2 = 4 * 1024 * 1024

export interface RunArtifactLocationV2 {
  canvasBranch: string
...
export interface ExcludedRunArtifactV2 {
  relativePath: string
  reason: 'control' | 'temporary' | 'symlink' | 'foreign' | 'unsupported' | 'limit'
}

export interface CloseRunArtifactsOptionsV2 {
  /** False when the run collector itself knows its output snapshot was incomplete. */
  complete?: boolean
}

export interface ClosedRunArtifactsV2 {
  location: RunArtifactLocationV2
  manifest: ArtifactManifestV1
  manifestDigest: string
  excluded: ExcludedRunArtifactV2[]
}

export interface RunArtifactLookupV2 extends ArtifactManifestEntryV1 {
  canvasBranch: string
  branchStorageId: string
  runId: string
...
export interface PluginArtifactClaimV2 {
  pluginId: string
  extensions?: string[]
  mediaTypes?: string[]
  priority?: number
...
export interface PluginArtifactMatchV2 {
  pluginId: string
  priority: number
  specificity: number
}
...
export class PluginArtifactClaimRegistryV2 {
  readonly #claims: PluginArtifactClaimV2[]

  constructor(claims: readonly PluginArtifactClaimV2[]) {
  ...
  matches(entry: ArtifactManifestEntryV1): PluginArtifactMatchV2[] {
  ...
  select(entry: ArtifactManifestEntryV1): PluginArtifactMatchV2 | undefined {
 ...
 */
export class RunArtifactStoreV2 {
  ...
  location(runId: string): RunArtifactLocationV2 {
    ...
    return {
      ...
      absoluteManifestPath: path.join(
        absoluteRunRoot,
        ...RUN_ARTIFACT_MANIFEST_RELATIVE_PATH_V2.split('/'),
      ),
    }
  }

  async prepareRun(runId: string): Promise<RunArtifactLocationV2> {
    return this.#runExclusive(async () => this.#prepareRun(runId))
  }

  async closeRun(
    runId: string,
    options: CloseRunArtifactsOptionsV2 = {},
  ): Promise<ClosedRunArtifactsV2> {
    return this.#runExclusive(async () => {
      const location = await this.#prepareRun(runId)
      const scan = await enumerateRunFiles(location)
      const manifest = buildArtifactManifestV1({
      ...
      if (existing) {
        const existingDigest = artifactManifestDigestV1(existing)
  ...
  async manifest(runId: string): Promise<ArtifactManifestV1 | undefined> {
    return this.#runExclusive(async () => {
  ...
  async lookup(runId: string, artifactId: string): Promise<RunArtifactLookupV2 | undefined> {
    return this.#runExclusive(async () => {
      const location = this.location(runId)
      ...
      return {
        ...
        absolutePath,
        manifestDigest: artifactManifestDigestV1(manifest),
      }
    })
  }
...
interface ArtifactScanV2 {
  files: ArtifactManifestFileCandidateV1[]
  excluded: ExcludedRunArtifactV2[]
  complete: boolean
}

async function enumerateRunFiles(location: RunArtifactLocationV2): Promise<ArtifactScanV2> {
  const files: ArtifactManifestFileCandidateV1[] = []
  const excluded: ExcludedRunArtifactV2[] = []
  let complete = true
  let visitedDirectories = 0

  const visit = async (directory: string, prefix: string, depth: number): Promise<void> => {
    if (depth > MAX_RUN_ARTIFACT_DEPTH_V2) {
      complete = false
      excluded.push({ relativePath: prefix, reason: 'limit' })
      return
    }
    visitedDirectories += 1
    if (visitedDirectories > MAX_RUN_ARTIFACT_DIRECTORIES_V2) {
      complete = false
      excluded.push({ relativePath: prefix || '.', reason: 'limit' })
      return
    }
    ...
    for (const entry of entries) {
      const relativePath = normalizeArtifactRelativePathV1(
      ...
      if (isTemporaryArtifactRelativePathV1(relativePath)) {
      ...
      if (files.length >= MAX_ARTIFACT_MANIFEST_ENTRIES_V1) {
        complete = false
...
async function readManifestFile(filePath: string): Promise<ArtifactManifestV1 | undefined> {
  ...
  try {
    ...
    if (!info.isFile() || info.size <= 0 || info.size > MAX_RUN_ARTIFACT_MANIFEST_BYTES_V2) {
    ...
    const inspection = inspectArtifactManifestV1(parsed)
...
async function verifyManifestEntry(
  ...
  const digest = await digestStableFile(filesRoot, absolutePath, info)
...
function resolveFilesRelative(filesRoot: string, relativePath: string): string {
  const normalized = normalizeArtifactRelativePathV1(relativePath)
...
function validateClaim(claim: PluginArtifactClaimV2, index: number): PluginArtifactClaimV2 {
