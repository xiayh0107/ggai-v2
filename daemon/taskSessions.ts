...
export const MAX_TASK_SESSION_RECORDS = 10_000
export const MAX_TASK_SESSIONS_FILE_BYTES = 8 * 1024 * 1024
export const MAX_TASK_SESSION_ID_LENGTH = 512
...
export interface TaskSessionUpsert {
  canvasBranch: string
  taskId: string
  agentId: string
  sessionId: string | null
...
export interface TaskSessionListFilter {
  canvasBranch?: string
  taskId?: string
  agentId?: string
}

export interface TaskSessionRecovery {
  filePath: string
...
export class TaskSessionsCorruptionError extends Error {
  readonly recovery: TaskSessionRecovery

  constructor(recovery: TaskSessionRecovery, cause?: unknown) {
    super(
      `Invalid task sessions file was quarantined at ${recovery.quarantinePath}; `
      + 'operator recovery is required before continuing',
      { cause },
    )
    this.name = 'TaskSessionsCorruptionError'
    this.recovery = recovery
  }
}

/** Canonical key for the strict canvasBranch + taskId + agentId identity. */
export function taskSessionKey(
  canvasBranch: string,
  taskId: string,
  agentId: string,
): string {
...
export class TaskSessionStore {
  ...
  #records: Map<string, TaskSessionRecord> | null = null
  #recovery: TaskSessionRecovery | null = null
  #operationTail: Promise<void> = Promise.resolve()

  constructor(filePath: string, options: TaskSessionStoreOptions = {}) {
  ...
  async get(
    ...
    return this.#runExclusive(async () => {
      ...
      const record = this.#records?.get(taskSessionKey(canvasBranch, taskId, agentId))
      return record ? { ...record } : undefined
    })
  }

  async list(filter: TaskSessionListFilter = {}): Promise<TaskSessionRecord[]> {
    return this.#runExclusive(async () => {
      await this.#preparePath()
      await this.#ensureLoaded()
  ...
  async upsert(input: TaskSessionUpsert): Promise<TaskSessionRecord> {
    return this.#runExclusive(async () => {
      await this.#preparePath()
      await this.#ensureLoaded()
      ...
      const key = taskSessionKey(canvasBranch, taskId, agentId)
  ...
  get recovery(): TaskSessionRecovery | null {
    return this.#recovery ? { ...this.#recovery } : null
  }

  async #ensureLoaded(): Promise<void> {
    if (this.#recovery) throw new TaskSessionsCorruptionError(this.#recovery)
    ...
    try {
      ...
      this.#recovery = recovery
      throw new TaskSessionsCorruptionError(recovery, error)
    }
  }
...
function parseRecords(source: string): Map<string, TaskSessionRecord> {
  if (Buffer.byteLength(source, 'utf8') > MAX_TASK_SESSIONS_FILE_BYTES) {
  ...
  if (entries.length > MAX_TASK_SESSION_RECORDS) {
    throw new TypeError('task sessions file has too many records')
  }

  const records = new Map<string, TaskSessionRecord>()
  for (const [storedKey, value] of entries) {
    ...
    assertTaskSessionRecord(record)
    const canonicalKey = taskSessionKey(record.canvasBranch, record.taskId, record.agentId)
...
function serializeRecords(records: ReadonlyMap<string, TaskSessionRecord>): string {
  if (records.size > MAX_TASK_SESSION_RECORDS) {
    throw new TypeError('task session store has too many records')
  }
  const document: Record<string, TaskSessionRecord> = Object.create(null) as Record<
    string,
    TaskSessionRecord
  >
  for (const [key, record] of [...records.entries()].sort(([left], [right]) =>
    ...
    assertTaskSessionRecord(record)
    if (key !== taskSessionKey(record.canvasBranch, record.taskId, record.agentId)) {
...
function assertTaskSessionRecord(record: TaskSessionRecord): void {
...
function parseSessionId(value: unknown): string | null {
  ...
  if (typeof value !== 'string'
    || value.length === 0
    || value.length > MAX_TASK_SESSION_ID_LENGTH
    || value !== value.trim()
    || value.startsWith('-')) {
...
function compareRecords(left: TaskSessionRecord, right: TaskSessionRecord): number {
...
async function readTextNoFollow(filePath: string): Promise<string> {
  ...
  try {
    ...
    if (!status.isFile()) throw new TypeError('task sessions path must be a regular file')
    if (status.size > MAX_TASK_SESSIONS_FILE_BYTES) {
      throw new TypeError('task sessions file exceeds the supported size')
    }
