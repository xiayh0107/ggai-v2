...
export type CanvasGitV2ErrorCode =
  | 'BRANCH_EXISTS'
  | 'BRANCH_NOT_FOUND'
...
export class CanvasGitV2Error extends Error {
  readonly code: CanvasGitV2ErrorCode
  readonly exitCode?: number

  constructor(
    code: CanvasGitV2ErrorCode,
    message: string,
    options: { cause?: unknown; exitCode?: number } = {},
  ) {
    super(message, options.cause === undefined ? undefined : { cause: options.cause })
    this.name = 'CanvasGitV2Error'
    this.code = code
    this.exitCode = options.exitCode
  }
}

export type CanvasGitV2Status =
...
export interface CanvasGitV2Worktree {
  id: string
  path: string
  branch: string | null
  commit: string
...
export interface CanvasGitV2Branch {
  name: string
  commit: string
  worktree: CanvasGitV2Worktree | null
}

export interface CanvasGitV2HistoryEntry {
  commit: string
  parents: string[]
  committedAt: string
  subject: string
...
export interface CanvasGitV2HistoryPage {
  entries: CanvasGitV2HistoryEntry[]
  nextCursor: string | null
}

export interface CanvasGitV2CheckpointInput {
  branch: string
  document: CanvasDocumentV2
  reason?: string
}

export interface CanvasGitV2CheckpointResult {
  branch: string
  commit: string
  changed: boolean
  status: Extract<CanvasGitV2Status, { state: 'ready' }>
}

export interface CanvasGitV2CreateBranchInput {
  name: string
  startPoint?: string
}

export interface CanvasGitV2RestoreInput {
  sourceBranch: string
  checkpoint: string
  newBranch: string
}

export interface CanvasGitV2MergeInput {
  sourceBranch: string
  targetBranch: string
}

export interface CanvasGitV2HistoryOptions {
  branch: string
  cursor?: string
  limit?: number
}

export interface CanvasGitV2StoreOptions {
  gitBinary?: string
  uuid?: () => string
  gitTimeoutMs?: number
}

export interface CanvasGitV2RecoverableMerge {
  branch: string
  commit: string
  firstParent: string
  secondParent: string
  document: CanvasDocumentV2
  firstParentDocument: CanvasDocumentV2
}
...
/**
 * V2-only Canvas history. It owns a repository and linked worktrees that are
 * physically disjoint from the legacy Canvas Git store.
 */
export class CanvasGitStoreV2 {
  ...
  constructor(projectDir: string, options: CanvasGitV2StoreOptions = {}) {
  ...
  async status(): Promise<CanvasGitV2Status> {
    return this.#runExclusive(() => this.#inspectStatus())
  }

  async checkpoint(input: CanvasGitV2CheckpointInput): Promise<CanvasGitV2CheckpointResult> {
    ...
    return this.#runExclusive(async () => {
      ...
      try {
        ...
        throw normalizeCodecError(error, 'Canvas V2 checkpoint is invalid')
      }
    })
  }

  async readDocument(refOrBranch: string): Promise<CanvasDocumentV2> {
    return this.#runExclusive(async () => {
      try {
  ...
  async listWorktrees(): Promise<CanvasGitV2Worktree[]> {
    return this.#runExclusive(async () => {
      await this.#requireInitialized()
  ...
  async listBranches(): Promise<CanvasGitV2Branch[]> {
    return this.#runExclusive(async () => {
      await this.#requireInitialized()
      ...
      const byBranch = new Map(
        worktrees
          .filter((worktree): worktree is CanvasGitV2Worktree & { branch: string } =>
      ...
      const branches = refs.stdout.split('\n').filter(Boolean).map((line) => {
        ...
        if (separator <= 0) {
          throw new CanvasGitV2Error(
            'GIT_COMMAND_FAILED',
            'Git returned malformed Canvas V2 branch data',
          )
        }
  ...
  async createBranch(input: CanvasGitV2CreateBranchInput): Promise<CanvasGitV2Branch> {
    return this.#runExclusive(async () => {
      try {
        await this.#requireInitialized()
        ...
        if (await this.#branchExists(input.name)) {
          throw new CanvasGitV2Error(
            'BRANCH_EXISTS',
            `Canvas V2 branch already exists: ${input.name}`,
          )
        }
        ...
        this.#recordFailure(error)
        throw normalizeCodecError(error, 'Canvas V2 branch start point is invalid')
      }
    })
  }

  async restoreAsNewBranch(input: CanvasGitV2RestoreInput): Promise<CanvasGitV2Branch> {
    return this.#runExclusive(async () => {
      try {
        await this.#requireInitialized()
        ...
        if (!(await this.#branchExists(input.sourceBranch))) {
          throw new CanvasGitV2Error(
            'BRANCH_NOT_FOUND',
            `Canvas V2 branch does not exist: ${input.sourceBranch}`,
          )
        }
        if (await this.#branchExists(input.newBranch)) {
          throw new CanvasGitV2Error(
            'BRANCH_EXISTS',
            `Canvas V2 branch already exists: ${input.newBranch}`,
          )
        }
        ...
        this.#recordFailure(error)
        throw normalizeCodecError(error, 'Canvas V2 restore checkpoint is invalid')
      }
    })
  }

  async history(options: CanvasGitV2HistoryOptions): Promise<CanvasGitV2HistoryPage> {
    return this.#runExclusive(async () => {
      try {
        await this.#requireInitialized()
        ...
        if (!(await this.#branchExists(options.branch))) {
          throw new CanvasGitV2Error(
            'BRANCH_NOT_FOUND',
            `Canvas V2 branch does not exist: ${options.branch}`,
          )
        }
  ...
  async previewMerge(input: CanvasGitV2MergeInput): Promise<GitMergePreview> {
    return this.#runExclusive(async () => {
      try {
  ...
  async merge(input: CanvasGitV2MergeInput): Promise<GitMergeExecution> {
    return this.#runExclusive(async () => {
      try {
        const metadata = await this.#mergeMetadata(input)
  ...
  ): Promise<CanvasGitV2RecoverableMerge | null> {
    return this.#runExclusive(async () => {
      try {
        await this.#requireInitialized()
        ...
        this.#recordFailure(error)
        throw normalizeCodecError(error, 'Canvas V2 recoverable merge is invalid')
      }
    })
  }

  async #mergeMetadata(input: CanvasGitV2MergeInput): Promise<MergeMetadata> {
    await this.#requireInitialized()
    await this.#requireHead()
    await this.#validateBranch(input.sourceBranch)
  ...
  async #attemptMerge(
    ...
    if (result.exitCode !== 0 && conflicts.length === 0) {
      ...
      throw new CanvasGitV2Error(
        'MERGE_FAILED',
        detail ? `Canvas V2 merge failed: ${detail}` : 'Canvas V2 merge failed',
    ...
    if (preview.state !== 'conflicts') {
      try {
        ...
        throw new CanvasGitV2Error(
          'INVALID_MERGE',
          'Git produced a clean but semantically invalid Canvas V2 merge',
          { cause: error },
        )
      }
    }
  ...
  async #withPreviewWorktree<T>(
    ...
    try {
      ...
      if (added) {
        ...
        await this.#assertCleanWorktree(worktreePath, 'Canvas V2 merge preview')
        if (locked) {
  ...
  async #assertCheckpointReady(
    ...
    if (status.length > 0) {
      throw new CanvasGitV2Error(
        'DIRTY_WORKTREE',
        `Canvas V2 worktree has uncommitted changes: ${branch}`,
      )
    }
  }
  ...
  async #assertCleanWorktree(worktreePath: string, label: string): Promise<void> {
    ...
    if (dirty.length > 0) {
      throw new CanvasGitV2Error(
        'DIRTY_WORKTREE',
        `Canvas V2 worktree has uncommitted changes: ${label}`,
      )
    }
  }
    ...
    for (let index = 0; index < entries.length; index += batchSize) {
      await Promise.all(entries.slice(index, index + batchSize).map(async (entry) => {
        if (!isManagedTreePath(entry.path)) {
          throw new CanvasGitV2Error(
            'INVALID_SNAPSHOT',
            `Codec returned an unmanaged Canvas V2 path: ${entry.path}`,
          )
        }
        const path = resolve(worktreePath, entry.path)
        if (!isPathWithin(worktreePath, path)) {
          throw new CanvasGitV2Error('UNSAFE_PATH', 'Canvas V2 snapshot path escapes worktree')
        }
  ...
  async #readDocumentFromTree(ref: string): Promise<CanvasDocumentV2> {
    ...
    for (const record of output.stdout.split('\0').filter(Boolean)) {
      ...
      if (metadata.length !== 3 || metadata[0] !== '100644' || metadata[1] !== 'blob') {
        throw new CanvasGitV2Error(
          'INVALID_SNAPSHOT',
          `Canvas V2 Git tree contains a non-regular entry: ${path || '<unknown>'}`,
        )
      }
  ...
  async #readDocumentFromWorktree(worktreePath: string): Promise<CanvasDocumentV2> {
    await this.#assertManagedWorktreePath(worktreePath)
    ...
    for (const record of index.stdout.split('\0').filter(Boolean)) {
      ...
      if (!match || match[1] !== '100644' || match[2] !== '0') {
        throw new CanvasGitV2Error(
          'INVALID_SNAPSHOT',
          'Canvas V2 worktree index contains a non-regular or unmerged entry',
        )
      }
      const path = match[3]
      if (!path || !isManagedTreePath(path)) {
        throw new CanvasGitV2Error(
          'INVALID_SNAPSHOT',
          `Canvas V2 worktree contains an unsupported indexed path: ${path ?? '<unknown>'}`,
        )
      }
    ...
    if (!sameStrings(actualPaths, indexedPaths)) {
      throw new CanvasGitV2Error(
        'INVALID_SNAPSHOT',
        'Canvas V2 worktree files do not exactly match its Git index',
      )
    }
  ...
  async #ensureInitialized(): Promise<void> {
    ...
    if (current === 'invalid') {
      throw new CanvasGitV2Error(
        'GIT_COMMAND_FAILED',
        `Canvas V2 state path is not a Git worktree: ${this.repoDir}`,
      )
    }
  ...
  async #requireInitialized(): Promise<void> {
    ...
    if (state === 'missing') {
      throw new CanvasGitV2Error(
        'REPOSITORY_EMPTY',
        'Canvas V2 Git history has not been initialized',
      )
    }
    if (state === 'invalid') {
      throw new CanvasGitV2Error(
        'GIT_COMMAND_FAILED',
        `Canvas V2 state path is not a valid Git worktree: ${this.repoDir}`,
      )
    }
  }

  async #inspectStatus(): Promise<CanvasGitV2Status> {
    try {
      await this.#git(['--version'], { cwd: this.projectDir })
    } catch (error) {
      const gitAvailable = !(error instanceof CanvasGitV2Error
    ...
    try {
      ...
      if (state === 'invalid') {
        const reason = `Canvas V2 state path is not a valid Git worktree: ${this.repoDir}`
  ...
  async #requireHead(): Promise<void> {
    if (!(await this.#hasHead(this.repoDir))) {
      throw new CanvasGitV2Error(
        'REPOSITORY_EMPTY',
        'Create the first Canvas V2 checkpoint before creating a branch',
      )
    }
  }
  ...
  async #validateBranch(branch: string): Promise<void> {
    if (
      ...
      throw new CanvasGitV2Error('INVALID_BRANCH', 'Invalid Canvas V2 branch name')
    }
    const result = await this.#git(
    ...
    if (result.exitCode !== 0) {
      throw new CanvasGitV2Error(
        'INVALID_BRANCH',
        `Invalid Canvas V2 branch name: ${branch}`,
      )
    }
  }
  ...
  async #resolveStartPoint(startPoint: string): Promise<string> {
    if (isCheckpointHash(startPoint)) {
      ...
      if (result.exitCode !== 0) {
        throw new CanvasGitV2Error(
          'CHECKPOINT_NOT_FOUND',
          'Canvas V2 checkpoint does not exist',
        )
      }
      return checkpoint
    }
    ...
    if (!(await this.#branchExists(startPoint))) {
      throw new CanvasGitV2Error(
        'BRANCH_NOT_FOUND',
        `Canvas V2 branch does not exist: ${startPoint}`,
      )
    }
  ...
  async #resolveReadableRef(refOrBranch: string): Promise<{ ref: string; branch: string | null }> {
    ...
    if (!(await this.#branchExists(refOrBranch))) {
      throw new CanvasGitV2Error(
        'BRANCH_NOT_FOUND',
        `Canvas V2 branch does not exist: ${refOrBranch}`,
      )
    }
  ...
  async #assertCheckpointOnBranch(checkpoint: string, branch: string): Promise<void> {
    ...
    if (ancestor.exitCode !== 0) {
      throw new CanvasGitV2Error(
        'CHECKPOINT_NOT_FOUND',
        `Checkpoint is not part of Canvas V2 branch ${branch}`,
      )
    }
  }

  async #createWorktree(branch: string, startPoint: string): Promise<CanvasGitV2Worktree> {
    await mkdir(this.worktreesDir, { recursive: true, mode: 0o700 })
    const id = managedUuid(this.#uuid())
    ...
    try {
      ...
      if (!worktree) {
        throw new CanvasGitV2Error(
          'GIT_COMMAND_FAILED',
          'Git did not report the new Canvas V2 worktree',
        )
      }
      return worktree
    } catch (error) {
  ...
  async #assertUnusedManagedWorktreePath(path: string): Promise<void> {
    if (!isPathWithin(this.worktreesDir, path) || path === this.worktreesDir) {
      throw new CanvasGitV2Error('UNSAFE_PATH', 'Generated Canvas V2 worktree path is unsafe')
    }
    try {
      await lstat(path)
      throw new CanvasGitV2Error(
        'UNSAFE_PATH',
        'Generated Canvas V2 worktree path already exists',
      )
    } catch (error) {
  ...
  async #worktreeForBranch(branch: string): Promise<CanvasGitV2Worktree> {
    if (!(await this.#branchExists(branch))) {
    ...
    if (!worktree) {
      throw new CanvasGitV2Error(
        'UNMANAGED_WORKTREE',
        `Canvas V2 branch has no active managed worktree: ${branch}`,
      )
    }
    if (!worktree.managed) {
      throw new CanvasGitV2Error(
        'UNMANAGED_WORKTREE',
        `Canvas V2 branch is checked out outside managed state: ${branch}`,
      )
    }
    return worktree
  }

  async #listWorktreesInternal(): Promise<CanvasGitV2Worktree[]> {
    const output = await this.#git(
  ...
  async #assertSafeManagedPaths(): Promise<void> {
    ...
    for (const path of [resolve(this.projectDir, '.gg'), this.repoDir, this.worktreesDir]) {
      ...
      if (!isPathWithin(canonicalProject, canonical)) {
        throw new CanvasGitV2Error(
          'UNSAFE_PATH',
          `Canvas V2 Git path escapes the project: ${path}`,
        )
      }
      try {
        ...
        if (info.isSymbolicLink()) {
          throw new CanvasGitV2Error(
            'UNSAFE_PATH',
            `Canvas V2 Git path must not be a symlink: ${path}`,
          )
        }
      } catch (error) {
        if (!isNodeError(error, 'ENOENT')) throw error
  ...
  async #assertManagedWorktreePath(path: string): Promise<void> {
    ...
    if (!primary && !linked) {
      throw new CanvasGitV2Error(
        'UNSAFE_PATH',
        `Worktree is outside managed Canvas V2 state paths: ${path}`,
      )
    }
    const info = await lstat(path)
    if (!info.isDirectory() || info.isSymbolicLink()) {
      throw new CanvasGitV2Error(
        'UNSAFE_PATH',
        `Managed Canvas V2 worktree must be a real directory: ${path}`,
      )
    }
  }
  ...
  #recordFailure(error: unknown): void {
    if (!(error instanceof CanvasGitV2Error)) return
...
function encodeSnapshot(document: CanvasDocumentV2): CanvasGitTreeEntryV2[] {
  try {
    return encodeCanvasGitTreeV2(document)
  } catch (error) {
    throw normalizeCodecError(error, 'Canvas V2 document cannot be checkpointed')
  }
}

function decodeSnapshot(entries: readonly CanvasGitTreeEntryV2[]): CanvasDocumentV2 {
  try {
    return decodeCanvasGitTreeV2(entries)
  } catch (error) {
    throw normalizeCodecError(error, 'Canvas V2 Git tree is invalid')
  }
}

function normalizeCodecError(error: unknown, message: string): unknown {
  return error instanceof CanvasGitCodecV2Error
    ? new CanvasGitV2Error('INVALID_SNAPSHOT', message, { cause: error })
    : error
}

function normalizeMergeError(error: unknown): unknown {
  if (error instanceof CanvasGitCodecV2Error) {
    return new CanvasGitV2Error(
      'INVALID_MERGE',
      'Canvas V2 merge failed semantic validation',
      { cause: error },
    )
  }
  return error
...
function checkpointSubject(reason: string | undefined): string {
  ...
  return normalized ? `canvas-v2: ${normalized}` : 'canvas-v2: checkpoint'
}

function mergeSubject(sourceBranch: string, targetBranch: string): string {
  return `canvas-v2: merge ${sourceBranch} into ${targetBranch}`.slice(0, 200)
}

function parseHistory(source: string): CanvasGitV2HistoryEntry[] {
  const fields = source.split('\0')
  if (fields.at(-1) === '') fields.pop()
  if (fields.length % 4 !== 0) {
    throw new CanvasGitV2Error(
      'GIT_COMMAND_FAILED',
      'Git returned malformed Canvas V2 history data',
    )
  }
  const entries: CanvasGitV2HistoryEntry[] = []
...
function parseWorktrees(source: string): PorcelainWorktree[] {
  return source.split('\0\0').filter(Boolean).map((record) => {
    ...
    if (!path) {
      throw new CanvasGitV2Error(
        'GIT_COMMAND_FAILED',
        'Git returned malformed Canvas V2 worktree data',
      )
    }
    return {
      path,
  ...
  for (const entry of topLevel) {
    ...
    if (entry.name === '.DS_Store') {
      if (!info.isFile() || info.isSymbolicLink()) {
        throw new CanvasGitV2Error('UNSAFE_PATH', `Ignored path is unsafe: ${path}`)
      }
      continue
    }
    if (info.isSymbolicLink()) {
      throw new CanvasGitV2Error(
        'UNSAFE_PATH',
        `Canvas V2 worktree path must not be a symlink: ${path}`,
      )
    }
    if ((MANAGED_ROOT_FILES as readonly string[]).includes(entry.name)) {
      if (!info.isFile()) {
        throw new CanvasGitV2Error('INVALID_SNAPSHOT', `Expected regular file: ${path}`)
      }
    ...
    if (!(MANAGED_DIRECTORIES as readonly string[]).includes(entry.name) || !info.isDirectory()) {
      throw new CanvasGitV2Error(
        'INVALID_SNAPSHOT',
        `Canvas V2 worktree contains an unsupported top-level path: ${entry.name}`,
      )
    }
    ...
    for (const child of children) {
      ...
      if (!childInfo.isFile() || childInfo.isSymbolicLink() || !isManagedTreePath(relative)) {
        throw new CanvasGitV2Error(
          'INVALID_SNAPSHOT',
          `Canvas V2 worktree contains an unsupported entity path: ${relative}`,
        )
      }
      paths.push(relative)
    }
...
function canonicalCheckpointHash(value: string): string {
  if (!isCheckpointHash(value)) {
    throw new CanvasGitV2Error(
      'CHECKPOINT_NOT_FOUND',
      'Invalid Canvas V2 checkpoint identifier',
    )
  }
...
function errorMessage(error: unknown): string {
  return error instanceof Error ? error.message : 'Canvas V2 Git operation failed'
}
