...
import { artifactIdForManifest } from '../artifactManifest.js'
import {
  openVerifiedRunArtifactFile,
  PluginArtifactClaimRegistry,
  RunArtifactStore,
...
test('uses the run-owned branch path and closes a verified canonical manifest', async () => {
  ...
  assert.equal(
    ...
    artifactIdForManifest('run-1', 'preview.png'),
  )
...
test('lookup verifies content and returns branch-isolated metadata', async () => {
  const projectDir = await temporaryProject()
  const main = new RunArtifactStore(projectDir, 'main')
  const feature = new RunArtifactStore(projectDir, 'feature/a')
  ...
  const artifactId = artifactIdForManifest('run-shared', 'result.txt')
...
test('descriptor-bound reads reject a symlink or regular-file replacement after lookup', async () => {
  const projectDir = await temporaryProject()
  const outsideDir = await temporaryProject()
  const store = new RunArtifactStore(projectDir, 'main')
  const location = await store.prepareRun('run-replaced-after-lookup')
  const artifactPath = path.join(location.absoluteFilesRoot, 'result.txt')
  const outsidePath = path.join(outsideDir, 'outside.txt')
  await writeFile(artifactPath, 'trusted', 'utf8')
  await writeFile(outsidePath, 'outside', 'utf8')
  const closed = await store.closeRun('run-replaced-after-lookup')
  const lookup = await store.lookup(
    'run-replaced-after-lookup',
    closed.manifest.entries[0]!.artifactId,
  )
  assert.ok(lookup)
  ...
  await assert.rejects(
    openVerifiedRunArtifactFile(lookup),
    /closed manifest|outside files root|symbolic link|ELOOP/u,
  )

  await rm(artifactPath)
  // Same byte length defeats a size-only check but must fail the digest check.
  await writeFile(artifactPath, 'hostile', 'utf8')
  await assert.rejects(
    openVerifiedRunArtifactFile(lookup),
    /digest|closed manifest/u,
  )
})

test('a verified descriptor keeps serving the manifest-bound inode after its path is replaced', async () => {
  const projectDir = await temporaryProject()
  const store = new RunArtifactStore(projectDir, 'main')
  const location = await store.prepareRun('run-descriptor-stable')
  const artifactPath = path.join(location.absoluteFilesRoot, 'result.txt')
  const movedPath = path.join(location.absoluteFilesRoot, 'moved.txt')
  await writeFile(artifactPath, 'trusted bytes', 'utf8')
  const closed = await store.closeRun('run-descriptor-stable')
  const lookup = await store.lookup(
    'run-descriptor-stable',
    closed.manifest.entries[0]!.artifactId,
  )
  assert.ok(lookup)

  const handle = await openVerifiedRunArtifactFile(lookup)
  try {
    await rename(artifactPath, movedPath)
    await writeFile(artifactPath, 'hostile bytes', 'utf8')
    const buffer = Buffer.alloc(lookup.size)
    const { bytesRead } = await handle.read(buffer, 0, buffer.length, 0)
    assert.equal(bytesRead, lookup.size)
    assert.equal(buffer.toString('utf8'), 'trusted bytes')
  } finally {
    await handle.close()
  }
})
...
test('plugin artifact claims are bounded and selected deterministically', () => {
  const registry = new PluginArtifactClaimRegistry([
  ...
  const entry = {
    artifactId: artifactIdForManifest('run-1', 'preview.png'),
  ...
  assert.throws(() => new PluginArtifactClaimRegistry([
  ...
  assert.throws(() => artifactIdForManifest('run-1', '../foreign.txt'), /unsafe/u)
})
