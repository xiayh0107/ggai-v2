...
function expectCodecError(
  ...
  assert.ok(caught instanceof CanvasGitCodecError)
...
function entryPath(entries: readonly CanvasGitTreeEntry[], prefix: string): string {
  const path = entries.find((entry) => entry.path.startsWith(prefix))?.path
  assert.ok(path)
  return path
}

test('encodes and reads a deterministic normalized Canvas Git tree', () => {
  const document = documentFixture()
  const entries = encodeCanvasGitTree(document)
  ...
  const decoded = readCanvasGitTree({
  ...
  assert.deepEqual(encodeCanvasGitTree(decoded), entries)
})
...
test('represents an empty canvas with only the managed root files', () => {
  ...
  assert.deepEqual(decodeCanvasGitTree(entries), document)
})
...
test('normalizes order-insensitive arrays without changing ordered payload arrays', () => {
  ...
  assert.deepEqual(encodeCanvasGitTree(reordered), encodeCanvasGitTree(original))
  const nodeEntry = encodeCanvasGitTree(reordered).find((entry) => {
...
test('uses receipt kind and plan ID together as the stable receipt identity', () => {
  ...
  const paths = encodeCanvasGitTree(document).map((entry) => entry.path)
  assert.ok(paths.includes(
    `receipts/${canvasGitEntityFileName('receipt', `materialization\0${OUTPUT_PLAN_ID}`)}`,
  ))
  assert.ok(paths.includes(
    `receipts/${canvasGitEntityFileName('receipt', `plan-dismissal\0${OUTPUT_PLAN_ID}`)}`,
  ))
})

test('does not admit runtime, log, selection, session, or view state', () => {
  const withRuntime = documentFixture() as CanvasDocument & Record<string, unknown>
  withRuntime.runtime = { activeRunId: 'run-private', sseCursor: 42 }
  expectCodecError(() => encodeCanvasGitTree(withRuntime), 'INVALID_DOCUMENT')
  ...
  expectCodecError(() => encodeCanvasGitTree(withNodePhase), 'INVALID_DOCUMENT')
})

test('rejects unknown roots, nested paths, case aliases, and unreadable entries', () => {
  const entries = encodeCanvasGitTree(documentFixture())
  const invalidPaths = [
  ...
  for (const path of invalidPaths) {
    expectCodecError(
      () => decodeCanvasGitTree([...entries, { path, content: '{}\n' }]),
      'INVALID_TREE',
    )
  }

  expectCodecError(
    () => readCanvasGitTree({ paths: ['.gitignore', 'meta.json'], read: () => undefined }),
    'INVALID_TREE',
  )
...
test('rejects malformed JSON, invalid metadata shape, and schema mismatches', () => {
  ...
  expectCodecError(
    () => decodeCanvasGitTree(replaceEntry(entries, nodePath, '{broken\n')),
    'MALFORMED_JSON',
  )
  expectCodecError(
    () => decodeCanvasGitTree(replaceEntry(
      entries,
      'meta.json',
...
test('rejects duplicate paths, identity tampering, and hash collision ambiguity', () => {
  ...
  expectCodecError(
    () => decodeCanvasGitTree([...entries, { ...nodeEntry }]),
    'DUPLICATE_PATH',
  )
  ...
  assert.notEqual(
    canvasGitEntityFileName('task', 'same-id'),
    canvasGitEntityFileName('node', 'same-id'),
  )
})
...
test('runs full semantic validation for dangling refs, cycles, and provenance', () => {
  const entries = encodeCanvasGitTree(documentFixture())
  ...
  expectCodecError(
    () => decodeCanvasGitTree(replaceEntry(entries, taskPath, `${JSON.stringify(task)}\n`)),
    'INVALID_DOCUMENT',
  )
  ...
  expectCodecError(() => decodeCanvasGitTree(withoutMaterialization), 'INVALID_DOCUMENT')
})
