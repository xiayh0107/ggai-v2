...
import { BUILTIN_ARTIFACT_CLAIM_REGISTRY } from '../../src/plugins/artifactContracts.js'
...
test('daemon built-ins are projected from the shared artifact claim registry', () => {
  assert.deepEqual(
    BUILTIN_PROJECTION_PLUGIN_CONTRACTS,
    BUILTIN_ARTIFACT_CLAIM_REGISTRY.map((registration) => ({
      id: registration.id,
...
test('daemon build artifact graph does not import browser renderer dependencies', async () => {
  const daemonModule = await readFile(new URL('../projectionPlugins.js', import.meta.url), 'utf8')
