...
function file(
  ...
  ownerRunId = 'run-1',
  overrides: Partial<ArtifactManifestFileCandidateV1> = {},
): ArtifactManifestFileCandidateV1 {
  return {
    ownerRunId,
    relativePath,
    kind: 'file',
...
function manifest(complete = true) {
  return buildArtifactManifestV1({
    runId: 'run-1',
    complete,
    files: [
...
function plugins(): ProjectionPluginContractV2[] {
  return [
    {
      id: 'code',
...
test('RunOutcome V2 permits empty actions and multiple primary outputs with bounded DAGs', () => {
  const valid = { ...validOutcome(), suggestedActions: [] }
  const inspected = inspectRunOutcomeV2(valid)
  ...
  const optionalInspection = inspectRunOutcomeV2(optionalDependencies)
  ...
  assert.equal(inspectRunOutcomeV2({
    ...valid,
    outputs: [
  ...
  assert.equal(inspectRunOutcomeV2({
    ...valid,
    outputs: Array.from({ length: MAX_RUN_OUTPUT_HINTS_V2 + 1 }, (_, index) => ({
      key: `output-${index}`,
  ...
  assert.equal(inspectRunOutcomeV2({
    ...valid,
    taskProposals: Array.from({ length: MAX_RUN_TASK_PROPOSALS_V2 + 1 }, (_, index) => ({
      key: `task-${index}`,
      title: `Task ${index}`,
...
test('RunOutcome V2 rejects Agent-authored authority, layout, payload, edge, and command fields', () => {
  const rootFields = [
    'runId',
    'taskId',
    'nodeId',
  ...
  for (const field of rootFields) {
    assert.equal(inspectRunOutcomeV2({ ...validOutcome(), [field]: 'forbidden' }).status, 'invalid')
  }
  ...
  for (const field of [
    ...
    assert.equal(inspectRunOutcomeV2(outcome).status, 'invalid')
  }
...
test('plugin projection claims are strict serializable data and fallback ranking is deterministic', () => {
  const inspection = inspectProjectionPluginContractsV2([{
    id: 'image',
  ...
  const first = buildProjectionPlanV2({
  ...
  const second = buildProjectionPlanV2({
    taskId: 'task-fallback',
    runId: 'run-fallback',
    runStatus: 'done',
    manifest: fallbackManifest,
...
test('built-in projection claims are serializable, include R, and retain a generic fallback', () => {
  const inspection = inspectProjectionPluginContractsV2(
    structuredClone(BUILTIN_PROJECTION_PLUGIN_CONTRACTS_V2),
  )
  assert.equal(inspection.status, 'valid')
...
test('different outputs may safely share one verified artifact reference', () => {
  const sharedManifest = buildArtifactManifestV1({
    runId: 'run-shared',
    complete: true,
  ...
  assert.equal(inspectRunOutcomeV2(sharedOutcome).status, 'valid')
  ...
  assert.equal(inspectProjectionPlanV2(result.plan).status, 'valid')
})

test('ArtifactManifestV1 has canonical runRoot/files semantics and excludes unsafe sources', () => {
  const subject = buildArtifactManifestV1({
    runId: 'run-1',
    complete: true,
    files: [
  ...
  assert.equal(subject.entries[0]?.artifactId, artifactIdForV1('run-1', 'keep.png'))
  assert.equal(
    artifactProjectPathV1(
  ...
  assert.match(artifactManifestDigestV1(subject), /^[0-9a-f]{64}$/u)
  assert.equal(inspectArtifactManifestV1(subject).status, 'valid')

  const tampered = structuredClone(subject)
  const originalDigest = artifactManifestDigestV1(subject)
  tampered.entries[0]!.contentDigest = 'd'.repeat(64)
  assert.equal(inspectArtifactManifestV1(tampered).status, 'valid')
  assert.notEqual(artifactManifestDigestV1(tampered), originalDigest)

  assert.throws(() => buildArtifactManifestV1({
    runId: 'run-1',
    complete: true,
...
test('ProjectionPlan is task/run-owned and intersects hints with manifest and plugin contracts', () => {
  ...
  assert.equal(inspectProjectionPlanV2(first.plan).status, 'valid')

  const tampered = structuredClone(first.plan)
  tampered.outputs[0]!.artifactRefs[0]!.runId = 'run-foreign'
  assert.equal(inspectProjectionPlanV2(tampered).status, 'invalid')

  assert.throws(() => buildProjectionPlanV2({
    taskId: 'task-1',
    runId: 'run-foreign',
    runStatus: 'done',
    manifest: manifest(),
...
test('ProjectionPlan bounds automatic materialization while permitting multiple primary outputs', () => {
  const count = MAX_AUTO_MATERIALIZED_OUTPUTS_V2 + 3
  const manyManifest = buildArtifactManifestV1({
    runId: 'run-many',
    complete: true,
  ...
  assert.equal(
    ...
    MAX_AUTO_MATERIALIZED_OUTPUTS_V2 - 9,
  )
  assert.equal(inspectProjectionPlanV2(result.plan).status, 'valid')
})

test('error, cancelled, and interrupted runs discard proposals and still produce partial plans', () => {
  for (const runStatus of ['error', 'cancelled', 'interrupted'] as ProjectionRunStatusV2[]) {
    const result = buildProjectionPlanV2({
      taskId: 'task-1',
      runId: 'run-1',
      runStatus,
  ...
  const incomplete = buildProjectionPlanV2({
    taskId: 'task-1',
    runId: 'run-1',
    runStatus: 'done',
