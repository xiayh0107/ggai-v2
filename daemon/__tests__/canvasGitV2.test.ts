...
function userNode(id: string, title: string): CanvasNodeV2 {
  return {
    id,
    type: 'text',
...
function userTask(id: string, title = id): CanvasTaskV2 {
  return {
    id,
    title,
    goal: `${title} goal`,
...
function expectV2Error(code: CanvasGitV2Error['code']): (error: unknown) => boolean {
...
test('checkpoints only normalized Canvas V2 entities and provides deterministic history', async () => {
  const projectDir = await temporaryProject()
  const store = new CanvasGitStoreV2(projectDir)
  ...
  assert.equal(page.entries[0]?.subject, 'canvas-v2: node-updated')
...
test('rejects runtime-shaped data before initializing or mutating Git', async () => {
  const projectDir = await temporaryProject()
  const store = new CanvasGitStoreV2(projectDir)
  const invalid = document() as CanvasDocumentV2 & Record<string, unknown>
  invalid.runtime = {
  ...
  await assert.rejects(
    ...
    expectV2Error('INVALID_SNAPSHOT'),
  )
...
test('creates UUID worktrees without putting logical branch names in paths', async () => {
  ...
  const store = new CanvasGitStoreV2(projectDir, { uuid: () => id })
...
test('strictly restores an old checkpoint as an isolated branch', async () => {
  ...
  const store = new CanvasGitStoreV2(projectDir, { uuid: () => ids.shift() ?? '' })
...
test('rejects malformed or unmanaged commit trees during reads and restores', async () => {
  ...
  await assert.rejects(store.readDocument(tampered), expectV2Error('INVALID_SNAPSHOT'))
  await assert.rejects(
...
test('aborts a clean Git merge whose combined document violates V2 semantics', async () => {
  const projectDir = await temporaryProject()
  const ids = [
...
test('refuses dirty managed worktrees and unsafe repository symlinks', async () => {
  ...
  const unsafe = new CanvasGitStoreV2(secondProject)
  await assert.rejects(
    unsafe.checkpoint({ branch: 'main', document: document() }),
    expectV2Error('UNSAFE_PATH'),
  )
