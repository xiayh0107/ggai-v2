...
test('persists exact task session records using a constructor-owned file path', async () => {
  ...
  assert.deepEqual(
    await new TaskSessionStore(filePath).get('main', 'task-1', '@openai/codex'),
    resumed,
  )

  const onDisk = JSON.parse(await readFile(filePath, 'utf8')) as Record<string, unknown>
  const key = taskSessionKey('main', 'task-1', '@openai/codex')
...
test('isolates sessions by both canvas branch and task id', async () => {
  const directory = await temporaryDirectory()
  const store = new TaskSessionStore(join(directory, 'sessions.json'))
...
test('serializes concurrent upserts without losing records', async () => {
  ...
  let now = 0
  const store = new TaskSessionStore(filePath, { now: () => (now += 1) })
  ...
  assert.equal((await new TaskSessionStore(filePath).list()).length, 40)
})
...
test('quarantines damaged data and remains explicitly blocked', async () => {
  ...
  const store = new TaskSessionStore(filePath)

  let corruption: TaskSessionsCorruptionError | undefined
  await assert.rejects(store.list(), (error: unknown) => {
    assert.ok(error instanceof TaskSessionsCorruptionError)
    corruption = error
    return true
  })
  assert.ok(corruption)
  assert.equal(await readFile(corruption.recovery.quarantinePath, 'utf8'), '{ invalid json')
  await assert.rejects(store.get('main', 'task-1', 'codex'), TaskSessionsCorruptionError)
  ...
  await assert.rejects(new TaskSessionStore(filePath).list(), TaskSessionsCorruptionError)
})
...
test('quarantines records whose stored key or exact schema is invalid', async () => {
  ...
  await writeFile(filePath, JSON.stringify({
    [taskSessionKey('main', 'task-1', 'codex')]: {
      canvasBranch: 'main',
      taskId: 'task-forged',
      agentId: 'codex',
      sessionId: 'session-1',
  ...
  const store = new TaskSessionStore(filePath)
  await assert.rejects(store.list(), TaskSessionsCorruptionError)
...
test('strictly rejects invalid branch, task, agent, session, and timestamps', async () => {
  ...
  assert.throws(() => taskSessionKey('main', 'task/escape', 'codex'), /taskId/u)
})
