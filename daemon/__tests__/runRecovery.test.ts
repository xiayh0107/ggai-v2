...
import {
  ...
  type InterruptedProjectionPlanStore,
} from '../runRecovery.js'
import { RunArtifactStore } from '../runArtifactStorage.js'
import { RunLogStore } from '../runLogs.js'

class PendingRecoveryPlans implements InterruptedProjectionPlanStore {
  readonly calls: Array<{ taskId: string; runId: string }> = []
  readonly pluginIds: string[][] = []
  readonly records = new Map<string, ProjectionPlanRecord>()
  ...
  async recoverInterrupted(
    input: Parameters<InterruptedProjectionPlanStore['recoverInterrupted']>[0],
  ): Promise<Awaited<ReturnType<InterruptedProjectionPlanStore['recoverInterrupted']>>> {
...
test('recovers Task Run manifests, partial plans, and closes while leaving archived runs unchanged', async () => {
  ...
  const artifactStores = new Map<string, RunArtifactStore>()
  const artifactStore = (branch: string) => {
    let store = artifactStores.get(branch)
    if (!store) {
      store = new RunArtifactStore(root, branch)
  ...
  try {
    ...
    const first = await recoverInterruptedTaskRuns({
      projectDir: root,
      runLogs,
      artifactStore,
    ...
    const second = await recoverInterruptedTaskRuns({
      projectDir: root,
      runLogs,
      artifactStore,
      projectionPlanStore: () => plans,
...
test('isolates a corrupt manifest and still recovers missing files for another run', async () => {
  ...
  const runLogs = new RunLogStore(root)
  const plans = new PendingRecoveryPlans()
  const stores = new Map<string, RunArtifactStore>()
  const artifactStore = (branch: string) => {
...
test('reuses an already-closed manifest but emits an interrupted partial plan', async () => {
  ...
  try {
    ...
    await recoverInterruptedTaskRuns({
      projectDir: root,
      runLogs,
      artifactStore: () => artifacts,
      projectionPlanStore: () => plans,
