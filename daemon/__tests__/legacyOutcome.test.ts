...
import {
  ...
  readLegacyRunOutcome,
  RUN_OUTCOME_RELATIVE_PATH,
} from '../legacyOutcome.js'
...
test('reads a bounded, exact v1 run outcome', async () => {
  ...
  try {
    ...
    assert.deepEqual(await readLegacyRunOutcome(subject.artifactDir), outcome())
  } finally {
    await subject.close()
  }
})
...
test('missing, malformed, oversized, and unknown outcomes are non-fatal', async (t) => {
  await t.test('missing', async () => {
    const subject = await fixture()
    try {
      assert.equal(await readLegacyRunOutcome(subject.artifactDir), undefined)
    } finally {
      await subject.close()
    }
  })
