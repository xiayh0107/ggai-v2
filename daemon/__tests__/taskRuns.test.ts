...
test('successful Task runs persist a trusted plan from the outcome sidecar and verified manifest', async () => {
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), 'ggai-task-runs-v2-plan-'))
  const root = await realpath(temporaryRoot)
  const artifactLocation = await new RunArtifactStore(root, 'main')
