...
import { autoMaterializeProjectionPlan } from '../canvasProjectionCoordinator.js'
import { CanvasCommandStoreManager } from '../canvasCommandStoreManager.js'
import { canvasBranchStorageId } from '../canvasBranch.js'
import { ProjectionPlanStore } from '../projectionPlanStore.js'
...
test('RunManager startup recovers Task Run by branch and isolates corrupt archived runs', async () => {
  const root = await temporaryRoot('ggai-manager-recovery-v2-')
  const logs = new RunLogStore(root)
  const featureArtifacts = new RunArtifactStore(root, 'feature/recovery')
  try {
    ...
    const corruptLocation = await new RunArtifactStore(root, 'main')
...
test('replayed startup materialization is idempotent and never revives Canvas entities', async () => {
  ...
  try {
    ...
    const onProjectionPlanReady = async (input: {
      plan: Parameters<typeof autoMaterializeProjectionPlan>[0]['plan']
      projectDir: string
      canvasBranch: string
    }) => {
      hookCalls += 1
...
function projectionStore(root: string, branch: string): ProjectionPlanStore {
  return new ProjectionPlanStore(path.join(
    root,
    '.gg',
    'runtime',
    'projection-plans',
