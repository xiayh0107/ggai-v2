...
export const RUN_OUTCOME_V2_RELATIVE_PATH = '.ggai/run-result.json'
export const MAX_RUN_OUTCOME_V2_BYTES = 1024 * 1024

const NO_FOLLOW_FLAG = process.platform === 'win32' ? 0 : constants.O_NOFOLLOW

/**
 * Reads an optional Agent-authored V2 sidecar from a daemon-selected files root.
 *
 ...
 */
export async function readRunOutcomeV2(
  filesRoot: string,
): Promise<RunOutcomeV2 | undefined> {
  ...
  try {
    ...
    if (outcomeInfo.size <= 0 || outcomeInfo.size > MAX_RUN_OUTCOME_V2_BYTES) return undefined
    ...
    try {
      ...
      if (!openedInfo.isFile()
        ...
        || openedInfo.nlink !== 1
        || openedInfo.size <= 0
        || openedInfo.size > MAX_RUN_OUTCOME_V2_BYTES) return undefined

      const buffer = Buffer.alloc(MAX_RUN_OUTCOME_V2_BYTES + 1)
      const { bytesRead } = await handle.read(buffer, 0, buffer.length, 0)
      const finalInfo = await handle.stat()
      if (bytesRead <= 0
        || bytesRead > MAX_RUN_OUTCOME_V2_BYTES
      ...
      const inspection = inspectRunOutcomeV2(JSON.parse(source) as unknown)
