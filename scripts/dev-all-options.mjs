...
/**
 * Resolve one Canvas model for both child processes. A single configured flag
 * is mirrored to the other side; contradictory flags fail before either
 * process starts, so local development cannot accidentally mix schemas.
 */
export function resolveCanvasModelEnvironment(environment = {}) {
  const daemon = parseCanvasModelFlag(
    environment.GGAI_CANVAS_MODEL_V2,
    'GGAI_CANVAS_MODEL_V2',
  )
  const frontend = parseCanvasModelFlag(
    environment.VITE_GGAI_CANVAS_MODEL_V2,
    'VITE_GGAI_CANVAS_MODEL_V2',
  )
  if (daemon && frontend && daemon !== frontend) {
    throw new Error(
  ...
  return {
    ...
    environment: {
      ...environment,
      GGAI_CANVAS_MODEL_V2: flag,
      VITE_GGAI_CANVAS_MODEL_V2: flag,
    },
  }
}
